...
class UILetterControl(tk.Frame):
    ...
    def __init__(self, master, action_callback=None):
        ...
        # FAIL POST
        self.btn_export = tk.Button(self, text=u"Экспорт", command=self.onClickExport)
        self.btn_export.grid({"column": 6, "row": 0, "sticky": "NSEW"})
    ...
    def onClickExport(self):
        if self.action_callback:
            self.action_callback("EXPORT", None)
