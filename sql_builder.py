...
# TODO: параметры для верификации базы
...
if defconf.USE_DAEMON:
    sql = sql_table("COMMAND_QUEUE",
                    ["uid serial", "command varchar(20)", "db_reestr_id integer unique", "db_letter_id integer",
                     "reestr_date date", "db_user_id text"], alter)
    dump_sql(sql, out, db)
###
if out:
    out.close()
