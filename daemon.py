...
def dowork(dbstorage, postconn, command):
    ...
    if command["command"] == "BARCODE_DEL":
        ...
        if err > "":
            log.error(
                "Daemon::dowork::BARCODE_DEL::get_reestr_info>>" + err)
            return
        letter_info, err = dbstorage.get_letter_info(command["db_letter_id"])
        if err > "":
            log.error(
                "Daemon::dowork::BARCODE_DEL::get_letter_info>>" + err)
            return
        commands.barcode_del(dbstorage, postconn, reestr_info, letter_info)
        res, err = dbstorage.delete_command(command)
        if err > "":
            log.error("Daemon::dowork::BARCODE_DEL::delete_command>>" + err)
    if command["command"] == "BARCODE_DEL_ALL":
        reestr_info, err = dbstorage.get_reestr_info(command["db_reestr_id"])
        if err > "":
            log.error(
                "Daemon::dowork::BARCODE_DEL_ALL::get_reestr_info>>" + err)
            return
        uinf, err = dbstorage.get_user_info(command["db_user_id"])
        if err > "":
            log.error(
                "Daemon::dowork::BARCODE_DEL_ALL::get_user_info>>" + err)
            return
        user_ident = UserIdentifier(uinf)
        ...
        if err > "":
            log.error(
                "Daemon::dowork::BARCODE_DEL_ALL::delete_command>>" + err)
    if command["command"] == "DATE":
        reestr_info, err = dbstorage.get_reestr_info(command["db_reestr_id"])
        if err > "":
            log.error("Daemon::dowork::DATE::get_reestr_info>>" + err)
            return
        commands.set_date(dbstorage, postconn, reestr_info,
                          command["reestr_date"])
        res, err = dbstorage.delete_command(command)
        if err > "":
            log.error("Daemon::dowork::DATE::delete_command>>" + err)
        ...
        try:
            ...
            for cmd, err in dbstorage.get_command_list():
                if err == "":
                    ...
                    # предотвращаем повторную обработку команды
                    if cmd["uid"] not in commands_in_process:
                        work_pool.apply_async(dowork,
                                              (dbstorage, postconn, cmd))
                else:
                    log.error("Daemon::main_loop::get_command_list>>" + err)
